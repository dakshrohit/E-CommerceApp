...
const Cart = () => {
  ...
  const handleIncQty = (item) => {
    ...
    toast.success(`${item.name || item.title} quantity increased`);
  ...
  const handleDecQty = (item) => {
    ...
    const newQty = item.quantity - 1;
    toast.success(`${item.name || item.title} quantity decreased`);
    updateCartItem(item.productId, newQty);
  };

  const handleRemove = (item) => {
    toast.success(`${item.name || item.title} removed from cart`);
